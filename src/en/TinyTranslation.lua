-- {"id":1141686301,"ver":"1.0.2","libVer":"1.0.2","author":"","repo":"","dep":[]}
...
local function parseNovel(novelURL)
    ...
    return NovelInfo({
        ...
        imageURL = imageURL,
        description = desc,
        chapters = AsList(mapNotNil(document:select(".entry-content ul > li > a"), function(v)
            local link = v:attr("href")
            if not link:find("tinytranslation") then return end
            return NovelChapter {
                order = v,
                title = v:text(),
                link = shrinkURL(link)
